...
author: Jeff Segovia
bannerUrl: /basic-form-react.png
...
export const bannerUrl = '/basic-form-react.png';
...
```tsx showLineNumbers
// working with forms using React hooks

import React from 'react';

function App() {
  // let's us create a simple markup first
  return (
    <form>
...
As you can see, each input of our form has a dedicated state and state setter. This approach is not good for bigger forms. Let's see now what we can do to improve our current implementation.
...
```tsx showLineNumbers {16-22, 32-33}
// working with forms using React hooks
...
With this approach, when we add another input, say an `email` and a `password` field, we don't have to change anything in our `handleChange` function. What we just need to update is our `fields` state.

```tsx showLineNumbers {11-12, 16}
// working with forms using React hooks
...
This is how simple it is to handle basic forms with ReactJS. For future posts, I'll try to address the common requirements of a better form handling implementation such as validation.
