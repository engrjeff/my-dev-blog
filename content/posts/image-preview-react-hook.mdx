...
Now there's much here. First, we're using HTML5's `input` with a type as `file`. We are also constraining the be able to only pick images from the system file dialog by providing `accept='image/*'`. Above the input is a container within which is an `img` element. We are providing a definite aspect-ratio to the container. Note that using aspect ratio is important to avoid layout shifts. One final thing to note is that the reason why we're just using `style` to give styles to our markup is that this is just an example component.
